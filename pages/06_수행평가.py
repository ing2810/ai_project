import streamlit as st  # ◀ 오타 수정: tf -> st
import pandas as pd
...
@st.cache_data
def load_data():
    # 동일 경로 혹은 상위 폴더 경로에 맞게 파일명을 지정하세요.
...
# 사용자 요청에 맞는 그라데이션 컬러맵 정의 (많은 쪽이 핑크, 적은 쪽이 블루)
pastel_pink_to_blue = ["#B3C5FF", "#FFB7B2"] # 파스텔 블루에서 파스텔 핑크로
...
themes = {
    ...
    "👑 영문 성명 & 브랜드 캐릭터": ["루이", "레오", "마리", "루비", "베리", "로이", "제니", "구찌", "샤넬", "디올", "미키", "루피", "토르", "코코", "토토", "미미", "나나", "밍키", "조이", "맥스"],
    "🧑 정감 가득 사람 이름 & 순우리말": ["두리", "철수", "순이", "영심이", "순자", "마루", "누리", "아리", "다온", "라온", "도담", "까치", "신비", "하나", "나비"]
}
